...
from elyra.kfp.operator import ExtendedContainerOp


@pytest.fixture
def notebook_op():
    return ExtendedContainerOp(name="test",
...
def test_fail_without_name():
    with pytest.raises(TypeError):
        ExtendedContainerOp(pipeline_name="test-pipeline",
                            experiment_name="experiment-name",
                            notebook="test_notebook.ipynb",
                            cos_endpoint="http://testserver:32525",
                            cos_bucket="test_bucket",
...
def test_properly_set_notebook_name_when_in_subdirectory():
    notebook_op = ExtendedContainerOp(name="test",
                                      pipeline_name="test-pipeline",
                                      experiment_name="experiment-name",
...
def test_normalize_label_value():
    ...
    # test min length
    assert ExtendedContainerOp._normalize_label_value(None) == ''
    assert ExtendedContainerOp._normalize_label_value('') == ''
    # test max length (63)
    assert ExtendedContainerOp._normalize_label_value('a' * 63) ==\
        'a' * 63
    assert ExtendedContainerOp._normalize_label_value('a' * 64) ==\
        'a' * 63  # truncated
    # test first and last char
    assert ExtendedContainerOp._normalize_label_value('1') == '1'
    assert ExtendedContainerOp._normalize_label_value('22') == '22'
    assert ExtendedContainerOp._normalize_label_value('3_3') == '3_3'
    assert ExtendedContainerOp._normalize_label_value('4u4') == '4u4'
    assert ExtendedContainerOp._normalize_label_value('5$5') == '5_5'
    ...
    for c in string.printable:
        if c in string.ascii_letters + string.digits:
            # first char is valid
            # no length violation
            assert ExtendedContainerOp._normalize_label_value(c) == c
            assert ExtendedContainerOp._normalize_label_value(c + 'B') == c + 'B'
            # max length
            assert ExtendedContainerOp._normalize_label_value(c + 'B' * 62) == \
                   (c + 'B' * 62)
            # max length exceeded
            assert ExtendedContainerOp._normalize_label_value(c + 'B' * 63) == \
                   (c + 'B' * 62)  # truncated
        else:
            ...
            if c not in valid_middle_chars:
                rv = '_'
            # no length violation
            assert ExtendedContainerOp._normalize_label_value(c) == 'a' + rv + 'a'
            assert ExtendedContainerOp._normalize_label_value(c + 'B') == 'a' + rv + 'B'
            # max length
            assert ExtendedContainerOp._normalize_label_value(c + 'B' * 62) == \
                   ('a' + rv + 'B' * 61)  # truncated
            # max length exceeded
    ...
    for c in string.printable:
        if c in string.ascii_letters + string.digits:
            # no length violation
            assert ExtendedContainerOp._normalize_label_value('b' + c) == 'b' + c
            # max length
            assert ExtendedContainerOp._normalize_label_value('b' * 62 + c) == \
                   ('b' * 62 + c)
            # max length exceeded
            assert ExtendedContainerOp._normalize_label_value('b' * 63 + c) == \
                   ('b' * 63)
        else:
            ...
            assert ExtendedContainerOp._normalize_label_value('b' + c) == 'b' + rv + 'a'
    ...
    for c in string.printable:
        if c in string.ascii_letters + string.digits:
            # no length violation
            assert ExtendedContainerOp._normalize_label_value(c + 'b' + c) == \
                   c + 'b' + c  # nothing is modified
            # max length
            assert ExtendedContainerOp._normalize_label_value(c + 'b' * 61 + c) == \
                   (c + 'b' * 61 + c)  # nothing is modified
            # max length exceeded
            assert ExtendedContainerOp._normalize_label_value(c + 'b' * 62 + c) == \
            ...
            assert ExtendedContainerOp._normalize_label_value(c + 'b' + c) ==\
                'a' + rv + 'b' + rv + 'a'
            # max length
            assert ExtendedContainerOp._normalize_label_value(c + 'b' * 59 + c) == \
                   ('a' + rv + 'b' * 59 + rv + 'a')
            # max length exceeded after processing, scenario 1
            # resolved by adding char before first, replace last
            assert ExtendedContainerOp._normalize_label_value(c + 'b' * 60 + c) == \
                   ('a' + rv + 'b' * 60 + 'a')
            ...
            assert ExtendedContainerOp._normalize_label_value(c + 'b' * 60 + c * 3) == \
                   ('a' + rv + 'b' * 60 + 'a')
    ...
    for c in string.printable:
        if c in string.ascii_letters + string.digits + '-_.':
            assert ExtendedContainerOp._normalize_label_value('A' + c + 'Z') ==\
                'A' + c + 'Z'
        else:
            assert ExtendedContainerOp._normalize_label_value('A' + c + 'Z') == 'A_Z'

    # encore
    assert ExtendedContainerOp._normalize_label_value(r'¯\_(ツ)_/¯') == 'a_________a'
